...
const RadarFullScreen = ({ stats, documents }) => {
  ...
  // NOVO: Calcular raio adaptativo baseado no número de categorias
  const getAdaptiveRadius = (numCategorias) => {
    if (numCategorias <= 6) return 40; // Raio normal
    if (numCategorias <= 10) return 42; // Pouco maior
    if (numCategorias <= 15) return 44; // Ainda maior
    return 46; // Máximo para muitas categorias
  };

  const radius = getAdaptiveRadius(categoriasList.length);

  // NOVO: Calcular tamanho dos cards baseado no número de categorias
  const getCardSize = (numCategorias) => {
    if (numCategorias <= 6) return { width: "160px", scale: "1" };
    if (numCategorias <= 10) return { width: "140px", scale: "0.95" };
    if (numCategorias <= 15) return { width: "130px", scale: "0.9" };
    return { width: "120px", scale: "0.85" };
  };

  const cardSize = getCardSize(categoriasList.length);
            ...
            return (
              <div
                ...
                style={{
                  left: `${x}%`,
                  top: `${y}%`,
                  transform: `translate(-50%, -50%) scale(${cardSize.scale})`,
                }}
              ...
              >
                ...
                >
                  ...
                  {/* Card */}
                  <div
                    className={`relative bg-slate-900/95 backdrop-blur-xl rounded-2xl border transition-all ${
                      isHovered
                        ? "border-emerald-500/50 shadow-2xl shadow-emerald-500/20"
                        : "border-slate-700/50 shadow-xl"
                    }`}
                    style={{ minWidth: cardSize.width }}
                  >
                    {/* Notification Badge */}
                    {hasNew && (
